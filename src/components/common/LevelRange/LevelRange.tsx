...
export const LevelRange: React.FC<ILevelRange> = ({ level, dynamic = false, size, onClick }) => {
    ...
    const isFilled = (index: number) => {
        const hoveredLevelInt = hoveredLevel ? parseInt(hoveredLevel, 10) : null;
        return dynamic ? (hoveredLevelInt !== null ? index <= hoveredLevelInt : index <= levelToInt) : index <= levelToInt;
    };
